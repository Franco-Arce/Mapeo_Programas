...
function calculateSimilarity(str1, str2) {
    ...
    if (withoutYear1.includes(withoutYear2) || withoutYear2.includes(withoutYear1)) return 90;

    // Word-overlap matching (65% threshold) - only significant words
    const STOPWORDS = new Set([
        'en', 'de', 'del', 'el', 'la', 'los', 'las', 'y', 'e', 'o', 'u',
        'para', 'por', 'con', 'sin', 'a', 'al', 'un', 'una', 'que', 'se',
        'su', 'nos', 'no', 'es', 'lo',
        'diplomado', 'curso', 'tec', 'tecnico', 'tecnica', 'tecnologia',
        'especializacion', 'maestria', 'licenciatura', 'ingenieria',
        'administracion', 'gestion', 'programa', 'carrera'
    ]);

    const sigWords1 = withoutYear1.split(/\s+/).filter(w => w.length > 2 && !STOPWORDS.has(w));
    const sigWords2 = withoutYear2.split(/\s+/).filter(w => w.length > 2 && !STOPWORDS.has(w));

    if (sigWords1.length > 0 && sigWords2.length > 0) {
        const set2 = new Set(sigWords2);
        const intersection = sigWords1.filter(w => set2.has(w));

        // Use the smaller set as reference
        const smallerLen = Math.min(sigWords1.length, sigWords2.length);
        const overlapRatio = intersection.length / smallerLen;

        if (overlapRatio >= 0.65 && intersection.length >= 1) {
            // Scale score: 65% overlap = 75, 100% overlap = 88
            const wordScore = 75 + (overlapRatio - 0.65) * (13 / 0.35);
            return Math.round(Math.min(wordScore, 88));
        }
    }
...
const COLUMN_PATTERNS = {
    ...
    contactId: ['idcontacto', 'dcontacto', 'id contacto', 'id_contacto', 'contacto_id', 'idinterno', 'id'],
...
function detectColumns(headers) {
    ...
    for (const header of headers) {
        ...
        // Email detection
        if (!detected.email && COLUMN_PATTERNS.email.some(p => normalizedHeader.includes(p))) {
            detected.email = header;
        }

        // Phone detection
        if (!detected.phone && COLUMN_PATTERNS.phone.some(p => normalizedHeader.includes(p))) {
            detected.phone = header;
        }

        // Contact ID detection - Check for exact patterns first, then combined
        if (!detected.contactId) {
            if (COLUMN_PATTERNS.contactId.some(p => normalizedHeader === normalize(p) || normalizedHeader.includes(normalize(p)))) {
                detected.contactId = header;
            } else if (normalizedHeader.includes('id') && normalizedHeader.includes('contacto')) {
                detected.contactId = header;
            }
        }

        // Database detection
        if (!detected.database && COLUMN_PATTERNS.database.some(p => normalizedHeader.includes(p))) {
...
function startMapping() {
    ...
    for (const [program] of state.programCounts) {
        ...
        if (score >= 85) {
            ...
            status = 'unmapped';
            mapped = match; // Guardar el mejor match encontrado para mostrarlo
        }
...
function renderResults(filter = 'all') {
    ...
    for (const [original, mapping] of state.mappings) {
        ...
        if (mapping.status === 'mapped') {
            ...
            const bestMatchInfo = mapping.mapped
                ? `<span class="unmapped">Sin mapear</span><br><span class="best-match-hint">Mejor coincidencia: <strong>${escapeHtml(mapping.mapped)}</strong> (${mapping.score}%)</span>`
                : `<span class="unmapped">Sin mapear</span>`;
            mappedCell = bestMatchInfo;
        }

        rows.push(`
            <tr data-original="${escapeHtml(original)}">
                <td class="original" data-label="Programa Original">${escapeHtml(original)}</td>
                <td data-label="Programa Mapeado">${mappedCell}</td>
                ...
                <td data-label="Registros">${count}</td>
                <td data-label="">
